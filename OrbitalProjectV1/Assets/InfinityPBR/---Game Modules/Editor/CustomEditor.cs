...
namespace InfinityPBR
{
    public abstract class CustomEditor<T> : Editor where T : Object
    {
        ...
        */
        ...
        {
            return Mathf.RoundToInt(EditorGUILayout.Slider((float) value, (float) min, (float) max));
        }
        
        public static int SliderInt(string label, int value, int min, int max)
        {
            return Mathf.RoundToInt(EditorGUILayout.Slider(label, (float) value, (float) min, (float) max));
        }
